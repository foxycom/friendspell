...
JAVA_OPTS=" -Dabc.instrument.array.operations -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.sqisland.friendspell "

INSTRUMENTATION_OPTS=" \
--skip-class=com.sqisland.friendspell.R \
...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.sqisland.friendspell$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.sqisland.friendspell.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall com.sqisland.friendspell > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall com.sqisland.friendspell.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" == "0" ]; then \
	...
	$(ADB) shell pm clear com.sqisland.friendspell > /dev/null; \
fi)
...
app-original.apk : 
	...
	mv app/build/outputs/apk/normal/debug/app-normal-debug.apk app-debug.apk && \
...
app-androidTest.apk :
	...
	mv app/build/outputs/apk/androidTest/normal/debug/app-normal-debug-androidTest.apk app-androidTest-unsigned.apk && \
...
app-original-for-coverage.apk app-androidTest-for-coverage.apk:
	...
	mv app/build/outputs/apk/normal/debug/app-normal-debug.apk app-original-for-coverage.apk  && \
	mv app/build/outputs/apk/androidTest/normal/debug/app-normal-debug-androidTest.apk app-androidTest-for-coverage.apk
...
#	Log directly to the expected file
	$(ADB) shell am instrument -w -e class $(TEST_NAME) com.sqisland.friendspell.test/com.sqisland.friendspell.util.MockTestRunner 2>&1 | tee $(@)
#	Copy the traces if the previous command succeded
	@export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces com.sqisland.friendspell ./traces/$(TEST_NAME) force-clean
